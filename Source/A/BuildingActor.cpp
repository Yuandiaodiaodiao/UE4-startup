...
ABuildingActor::ABuildingActor()
{
	...
	BuildingList.Add(ConstructorHelpers::FClassFinder<ATower>(TEXT("/Game/Blueprints/BTower")).Class);
	BuildingList.Add(ConstructorHelpers::FClassFinder<ATower>(TEXT("/Game/Blueprints/BTower1")).Class);
}
...
void ABuildingActor::NextBuilding()
{
	...
	//     Child->Destroy();
	// }
	ATower* NewActor = GetWorld()->SpawnActor<ATower>(BuildingList[BuildingId], Location, FRotator(0),
...
void ABuildingActor::PutBuilding()
{
	...
	// 	auto player = Cast<AACharacter>(pawn);
	// 	// player->genBuildingActor();
	// 	gened = true;
	// 	UE_LOG(LogTemp, Error, TEXT("return put"));
	//
	// 	return;
	// }
	UE_LOG(LogTemp, Error, TEXT("startput"));
	...
	FString classname=TowerDataCore->TowerClass->GetName();
	FString location=TowerDataCore->Location.ToString();
	UE_LOG(LogTemp, Error, TEXT("%s %s "),*classname,*location);
	//传入world 创建actor
